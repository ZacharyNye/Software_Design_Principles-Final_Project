/**
 * @author znye
 * @createdOn 12/1/2025 at 6:14 PM
 * @projectName Software_Design_Principles-Final_Project
 * @packageName edu.neumont.csc360.decorator;
 */
package edu.neumont.csc360.decorator;

import edu.neumont.csc360.model.Vehicle;
import edu.neumont.csc360.strategy.FuelStrategy;

public abstract class VehicleDecorator implements Vehicle {
    protected Vehicle decoratedVehicle;

    public VehicleDecorator(Vehicle decoratedVehicle) {
        this.decoratedVehicle = decoratedVehicle;
    }

    @Override
    public String getType() {
        return decoratedVehicle.getType();
    }

    @Override
    public int getHorsePower() {
        return decoratedVehicle.getHorsePower();
    }

    @Override
    public void setFuelStrategy(FuelStrategy strategy) {
        decoratedVehicle.setFuelStrategy(strategy);
    }

    @Override
    public double computeFuelUsage(int distance) {
        return decoratedVehicle.computeFuelUsage(distance);
    }
}

